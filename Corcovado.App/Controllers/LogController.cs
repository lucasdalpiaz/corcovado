    ...
    public class LogController : ApiController
    {
        ...
        public  static async void RegistraLogComDiferencaMaiorQue(int diferenca)
        {
            try
            {
                ...
                id,
                data_pos,
                LEAD (id,1) OVER (PARTITION BY mobile ORDER BY data_pos desc) AS id_anterior,
                LEAD (data_pos,1) OVER (PARTITION BY mobile ORDER BY data_pos desc) AS data_pos_anterior,
                NOW() as data_criacao
                FROM tb_dpsync
                )
            ...
            try
            {
                ...
                using (var ctx = new DPSyncContext())
                {
                    foreach (DateTime dia in listDiasAnteriores)
                    {
                        ...
                        int barcosCont = ctx.barcos.Where(x=>x.esn_global != "D").Count();
